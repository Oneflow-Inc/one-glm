...
# FLOAT_TYPES = (flow.FloatTensor, flow.cuda.FloatTensor)
# HALF_TYPES = (flow.HalfTensor, flow.cuda.HalfTensor)
...
#         model = flow.nn.Linear(D_in, D_out).cuda().half()
...
#     Args:
#         init_optimizer (flow.optim.optimizer):  Existing optimizer created with the parameters to optimize.  Internally, :class:`FP16_Optimizer` replaces the passed optimizer's fp16 parameters, if any, with fp32 master parameters copied from the original ones.  :class:`FP16_Optimizer` also stores references to the original fp16 parameters, and updates these fp16 parameters from the master fp32 copy at the end of each :attr:`step`.  
...
#                     if param.type() == 'flow.cuda.HalfTensor':
#                         self.maybe_print("FP16_Optimizer received flow.cuda.HalfTensor with {}"
#                                          .format(param.size()))
...
#                     elif param.type() == 'flow.cuda.FloatTensor':
#                         self.maybe_print("FP16_Optimizer received flow.cuda.FloatTensor with {}"
#                                          .format(param.size()))
#                         fp32_params_this_group.append(param)
#                         param_group['params'][i] = param
#                     else:
#                         raise TypeError("Wrapped parameters must be either "
#                                         "flow.cuda.FloatTensor or flow.cuda.HalfTensor. "
...
#                         param.grad.detach_()  # as in flow.optim.optimizer.zero_grad()
#                         param.grad.zero_()
...
#         """
#         Clips fp32 master gradients via ``flow.nn.utils.clip_grad_norm``.
...
#             flow.save(checkpoint, "saved.pth")
#         """
#         state_dict = {}
...
#             optimizer = FP16_Optimizer(optimizer, static_loss_scale = 128.0)
#             ...
#             checkpoint = flow.load("saved.pth")
...
#             http://pyflow.org/docs/master/optim.html#optimizer-step-closure
#         """
