...
def test_set_cuda_rng_state(model_parallel_size):
    ...
    size = 123
    seed = 1234
    flow.cuda.manual_seed(1234)
    tensor = flow.cuda.FloatTensor(size)

    # Get the state
    rng_state = flow.cuda.get_rng_state()
    rng_state_copy = rng_state.clone()

    # Do some stuff.
    for _ in range(5):
        flow.randn(size, out=tensor)
    result_1 = tensor.clone()

    assert rng_state.sub(rng_state_copy).max() == 0
    assert flow.cuda.get_rng_state().sub(rng_state_copy).max() > 0

    # State should be different.
    new_rng_state = flow.cuda.get_rng_state()
    max_diff = new_rng_state.sub(rng_state).max()
    print('   max diff in rng state (should be non-zero) on global rank {}: {}'.
          format(flow.distributed.get_rank(), max_diff))
    assert max_diff > 0
    ...
    print('   max error in generated tensors (should be zero) on '
          'global rank {}: {}'.format(flow.distributed.get_rank(), error))
    assert error < 1.0e-6
...
def test_cuda_rng_tracker(model_parallel_size):
    ...
    print('   max diff in generated tensors (should be non-zero) on '
          'global rank {}: {}'.format(flow.distributed.get_rank(), diff))
    assert diff > 1.0e-6
...
def test_model_parallel_cuda_manual_seed(model_parallel_size):
    ...
    assert flow.cuda.initial_seed() == 12345
    with mpu.get_cuda_rng_tracker().fork():
        assert flow.cuda.initial_seed() == (12345 + 2718 +
