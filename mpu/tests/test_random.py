...
def test_set_cuda_rng_state(model_parallel_size):
    ...
    print('   max diff in rng state (should be non-zero) on global rank {}: {}'.
          format(torch.env.get_rank(), max_diff))
    assert max_diff > 0
    ...
    print('   max error in generated tensors (should be zero) on '
          'global rank {}: {}'.format(torch.env.get_rank(), error))
    assert error < 1.0e-6

    # Input state should have remained intact.
    error = rng_state.sub(rng_state_copy).max()
    print('   max error in rng state (should be zero) on global rank {}: {}'.
          format(torch.env.get_rank(), error))
    assert error == 0
...
def test_cuda_rng_tracker(model_parallel_size):
    ...
    print('   max diff in generated tensors (should be non-zero) on '
          'global rank {}: {}'.format(torch.env.get_rank(), diff))
    assert diff > 1.0e-6
