...
def test_initialize_model_parallel(model_parallel_size):
    ...
    def check(group, world_size, rank):
        assert world_size == torch.env.get_world_size(group=group)
        assert rank == torch.env.get_rank(group=group)

    # Model parallel.
    world_size = model_parallel_size_
    rank = torch.env.get_rank() % model_parallel_size_
    ...
    # Data parallel.
    world_size = torch.env.get_world_size() // model_parallel_size_
    rank = torch.env.get_rank() // model_parallel_size
...
def test_get_model_parallel_src_rank(model_parallel_size_):
    ...
    # Checks
    src_rank = torch.env.get_rank() - mpu.get_model_parallel_rank()
