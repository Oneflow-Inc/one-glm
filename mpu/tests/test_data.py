...
def test_boradcast_data(model_parallel_size):
    ...
    flow.manual_seed(1234 + mpu.get_data_parallel_rank())
    ...
    for key in key_size_t:
        data[key] = flow.LongTensor(size=key_size_t[key]).random_(0, 1000)
        data_t[key] = data[key].clone()
    data['keyX'] = flow.FloatTensor(size=(5, )).random_(0, 1000)
    data_t['keyX'] = data['keyX'].clone()
    if mpu.get_model_parallel_rank() != 0:
        data = None

    data_utils._check_data_types(keys, data_t, flow.int64)
    key_size, key_numel, \
    ...
    data_b = data_utils.broadcast_data(keys, data, flow.int64)
    for key in keys:
