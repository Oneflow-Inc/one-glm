...
def test_parallel_embedding(model_parallel_size):
    ...
    set_random_seed(123)
    input_data = flow.LongTensor(
        size=(batch_size,seq_length)).random_(0, vocab_size).cuda()
    loss_weight = flow.randn([batch_size, seq_length, hidden_size]).cuda()

    set_random_seed(seed)
    embedding_original = flow.nn.Embedding(vocab_size, hidden_size).cuda()

    output = embedding_original(input_data)
    loss_original = flow.mul(output, loss_weight).sum()
    ...
    loss_parallel = flow.mul(output, loss_weight).sum()
    ...
    loss_vocab_parallel = flow.mul(output, loss_weight).sum()
    ...
    weight_grad_orig = flow.split(embedding_original.weight.grad,
...
def test_initialize_affine_weight(model_parallel_size):
    ...
    # Column parallel
    # ---------------
    weight = flow.empty(output_size_coeff, input_size)
    set_random_seed(seed)
    ...
    flow.nn.init.normal_(master_weight)
    rank = mpu.get_model_parallel_rank()
    my_weight = flow.split(master_weight, output_size_coeff,
    ...
    print('   column parallel max error (should be zero) on global rank '
          '{}: {}'.format(flow.distributed.get_rank(), error))
    assert error < 1.0e-6

    # ------------
    # Row parallel
    # ------------
    weight = flow.empty(output_size, input_size_coeff)
    set_random_seed(seed)
    ...
    flow.nn.init.normal_(master_weight)
    rank = mpu.get_model_parallel_rank()
    my_weight = flow.split(master_weight, input_size_coeff,
...
class IdentityLayer2D(flow.nn.Module):
    def __init__(self, m , n):
        super(IdentityLayer2D, self).__init__()
        self.weight = Parameter(flow.Tensor(m, n))
        flow.nn.init.xavier_normal_(self.weight)
    def forward(self):
        return self.weight
...
def test_column_parallel_linear(model_parallel_size):
    ...
    loss_weight = flow.randn([batch_size, output_size]).cuda()
    # Forward
    input_ = identity_layer()
    output = linear_layer(input_)
    loss = flow.mul(output, loss_weight).sum()
    # Backward
    loss.backward()
    ...
    dLdA = flow.matmul(dLdY.t(), X)
    dLdb = flow.matmul(flow.ones(batch_size, 1).cuda().t(), dLdY).view(-1)
    dLdX = flow.matmul(dLdY, A)

    rank = mpu.get_model_parallel_rank()
    my_dLdA = flow.split(dLdA, output_size_coeff,
                          dim=0)[rank].contiguous().clone()
    ...
    my_dLdb = flow.split(dLdb, output_size_coeff,
                          dim=0)[rank].contiguous().clone()
...
def test_row_parallel_linear(model_parallel_size):
    ...
    rank = mpu.get_model_parallel_rank()
    my_dLdA = flow.split(dLdA, input_size_coeff,
                          dim=1)[rank].contiguous().clone()
    error = my_dLdA.sub(linear_layer.weight.grad).abs().max()
    flow.distributed.barrier()
    print('   error in dLdA on global rank {}: {}'.format(
...
class IdentityLayer3D(flow.nn.Module):
    def __init__(self, m , n, k):
        super(IdentityLayer3D, self).__init__()
        self.weight = Parameter(flow.Tensor(m, n, k))
        flow.nn.init.xavier_normal_(self.weight)
    def forward(self):
        return self.weight
...
def parallel_self_attention(model_parallel_size, num_att_heads_per_partition,
    ...
    attention_layer = mpu.BertParallelSelfAttention(hidden_size, num_att_heads,
                                                dropout_prob).cuda()
    loss_weight = flow.randn([batch_size, sequence_length, hidden_size]).cuda()
    attention_mask = flow.randn([batch_size, 1, 1, sequence_length]).cuda()
    # Forward
    input_ = identity_layer()
...
def parallel_transformer(model_parallel_size, num_att_heads_per_partition,
    ...
    transformer_layer = mpu.BertParallelTransformerLayer(
        ...
        flow.nn.functional.relu, 1.0e-5).cuda()
